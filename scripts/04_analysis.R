...
# Retrieve the top 5 up-regulated and 5 down-regulated significant genes per pairwise contrast
get_top_degs <- function(res_object, contrast_name, n = 5) {
  ...
  # Convert ORF → standard gene name
  gene_map <- AnnotationDbi::select(
    org.Sc.sgd.db,
    keys = c(top_up$ORF, top_down$ORF),
    keytype = "ORF",
    columns = c("GENENAME")
  )
  
  top_up <- merge(top_up, gene_map, by = "ORF", all.x = TRUE)
  top_down <- merge(top_down, gene_map, by = "ORF", all.x = TRUE)
...
top_thin_early <- get_top_degs(resLFC_thin_early, "Thin vs Early")
top_mature_thin <- get_top_degs(resLFC_mature_thin, "Mature vs Thin")
top_mature_early <- get_top_degs(resLFC_mature_early, "Mature vs Early")

top_thin_early$up$GENENAME
top_thin_early$down$GENENAME
top_mature_thin$up$GENENAME
top_mature_thin$down$GENENAME
top_mature_early$up$GENENAME
top_mature_early$down$GENENAME
...
gene_map <- AnnotationDbi::select(org.Sc.sgd.db,
                                  keys = gene_names,
                                  keytype = "ORF",
                                  columns = c("GENENAME"))

# Merge and reorder to match heatmap order
gene_map <- gene_map[match(gene_names, gene_map$ORF), ]

# Replace rownames in heatmap matrix
rownames(mat) <- ifelse(is.na(gene_map$GENENAME),
                        gene_names,
                        gene_map$GENENAME)
...
go_thin_early <- run_go_enrichment(resLFC_thin_early, "Thin vs Early")
go_mature_thin <- run_go_enrichment(resLFC_mature_thin, "Mature vs Thin")
go_mature_early <- run_go_enrichment(resLFC_mature_early, "Mature vs Early")

# Number of enrichment GO BP terms above threshold 
nrow(as.data.frame(go_thin_early))
# 22
nrow(as.data.frame(go_mature_thin))
# 22
nrow(as.data.frame(go_mature_early))
#66
...
ggsave(
  "GO_enrichment_combined2.png",
  plot = combined_plot,
  width = 16,      # <-- Make it wide
  height = 6,      # Not too tall
  dpi = 300
)
